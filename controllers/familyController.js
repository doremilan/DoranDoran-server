...
const searchUser = async (req, res) => {
  try {
    ...
    const findEmail = User.email.find({ $text: { $search: search } });
    ...
    // user schema 의 toHexstring() 부분에서 에러가 났음.
    // user DB에 _id 가 없는 데이터가 있어서 이를 더미값으로 판단해서 에러가 났던 것.

    //지금은 유지하되, 나온 검색 결과가 DB에서 그 값이 완전히 일치했을 때만, josn형식으로 보내주기.
    ...
    //ssearchKeyword에서 나온 배열에 속한 것들 중 하나가
    //User db에서 findOne({search})으로 찾은 하나의 email과
    //완전 일치하는 것만 보낸다.
    // const exEmail = searchKeyword.email;
    // console.log("exEmail-->", exEmail);
    // const exUser = User.findOne({ exEmail: exEmail });
