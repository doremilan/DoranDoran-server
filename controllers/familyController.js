...
const editFamilyMember = async (req, res) => {
  try {
    ...
    await FamilyMember.updateOne({ familyId, _id: familyMemberId }, { $set: { familyMemberNickname } })
    const modifyFamilyMemberList = await FamilyMember.findOne({
      familyId,
      _id: familyMemberId,
    })
    // 음성 파일 수정
    await VoiceFile.updateMany({ familyId, userId: modifyFamilyMemberList.userId }, { $set: { familyMemberNickname } })
    // 미션멤버 수정
    await MissionMember.updateMany({ familyId, userId: modifyFamilyMemberList.userId }, { $set: { familyMemberNickname } })
