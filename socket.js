...
module.exports = server => {
  ...
  io.on("connection", socket => {
    ...
    socket.on("inviteMember", async ({ familyId, selectEmail, familyMemberNickname, nickname, type }) => {
      console.log(4, familyId, selectEmail, familyMemberNickname, nickname, type)
    ...
    socket.on("sendLikeNoti", async ({ photoId, senderName, senderId, receiverId, type, category, likeChk }) => {
      console.log(1, photoId, senderName, senderId, receiverId, type, category, likeChk)
      // DB 생성
      if (senderName !== undefined && senderId !== undefined && senderId !== receiverId) {
        const createdAt = new Date()
        if (likeChk) {
          await Alert.create({
            photoId,
    ...
    socket.on("sendCommentNoti", async ({ photoId, senderName, senderId, receiverId, type, category }) => {
      console.log(2, photoId, senderName, senderId, receiverId, type, category)
      // DB 생성
      if (senderName !== undefined && senderId !== undefined && senderId !== receiverId) {
        const createdAt = new Date()
        await Alert.create({
